  ...
  for(let attempt=1;attempt<=2;attempt++){
    ...
    try{
      const r=await fetch(cfg.url,{
        method:'POST',
        headers:{'Content-Type':'application/json','x-catalog-sync-secret':cfg.secret},
        body:JSON.stringify({action,...payload}),
        signal:controller?controller.signal:undefined
      });
      const txt=await r.text();let data={};try{data=JSON.parse(txt)}catch(_){data={error:txt}};
      if(!r.ok||!data.ok){
        const error=new Error(data.error||`Catálogo remoto HTTP ${r.status}`);
      ...
      const transient=!status||[408,425,429,500,502,503,504].includes(status)||error&&error.name==='AbortError';
      if(attempt<2&&transient){await new Promise(resolve=>setTimeout(resolve,700));continue;}
      if(error&&error.name==='AbortError')throw new Error('El catálogo remoto tardó demasiado en responder.');
      throw error;
    }finally{if(timer)clearTimeout(timer);}
  }
  throw lastError||new Error('No se pudo conectar con el catálogo remoto.');
}
...
async function saveSingleProductRemote(rawProduct,actor){
  const loaded=await remoteCall('load');
  const catalog=normalizeCatalog(loaded&&loaded.catalog||{});
  const incoming=product(rawProduct||{},0);
  const index=catalog.products.findIndex(item=>item.id===incoming.id);
  if(index>=0)catalog.products[index]=incoming;else catalog.products.push(incoming);
  const errors=validate(catalog);
  if(errors.length)throw new Error(errors.join('\n'));
  const data=await remoteCall('save',{catalog,actor:actor||'sublichat'});
  return normalizeCatalog(data&&data.catalog||catalog);
}

async function saveSingleProductDirect(rawProduct,actor){
  const db=getCatalogDb();
  const ref=db.collection(process.env.CATALOG_COLLECTION||'catalogo').doc(process.env.CATALOG_DOCUMENT||'publico');
  const before=await ref.get();
  const catalog=normalizeCatalog(before.exists?before.data():{});
  const incoming=product(rawProduct||{},0);
  const index=catalog.products.findIndex(item=>item.id===incoming.id);
  if(index>=0)catalog.products[index]=incoming;else catalog.products.push(incoming);
  const errors=validate(catalog);
  if(errors.length)throw new Error(errors.join('\n'));
  catalog.catalogVersion=Math.max(1,Number(catalog.catalogVersion)||1)+1;
  catalog.updatedAt=new Date().toISOString();
  await db.runTransaction(async tx=>{
    tx.set(ref,catalog,{merge:false});
    const h=db.collection('catalogoHistorial').doc();
    tx.set(h,{actor:actor||'sublichat',catalogVersion:catalog.catalogVersion,previousVersion:before.exists?(Number(before.data()?.catalogVersion)||null):null,productCount:catalog.products.length,promotionCount:catalog.promotions.length,createdAt:admin.firestore.FieldValue.serverTimestamp()});
  });
  return catalog;
}
...
async function handler(req,res){
  ...
  try{
    ...
    if(action==='guardar_producto'){
      if(!body.product||typeof body.product!=='object')return res.status(400).json({ok:false,error:'Falta el producto a guardar.'});
      const catalog=remote
        ?await saveSingleProductRemote(body.product,editor.actor||editor.kind)
        :await saveSingleProductDirect(body.product,editor.actor||editor.kind);
      return res.status(200).json({ok:true,catalog,message:`Producto “${clean(body.product.name,120)||'Producto'}” publicado · versión ${catalog.catalogVersion}`,source:remote?'remote':'firestore'});
    }
